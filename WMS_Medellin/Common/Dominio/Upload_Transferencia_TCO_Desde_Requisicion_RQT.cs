...
namespace Dominio
{
    public class Upload_Transferencia_TCO_Desde_Requisicion_RQT : IUpload
    {
        public void procesar(EstructuraUploadModel objEnvio)
        {
                ...
                try
                {
                    ...
                    foreach (DataRow registro in objEnvio.dtMovimientos.Rows)
                    {
                        if (Information.IsNumeric(registro["CantidadCompromiso"].ToString()))
                        {
                            ...
                            strXML_GT += "<F_CIA>" + objEnvio.cia_Destino + "</F_CIA>" + Environment.NewLine;
                            strXML_GT += "<f440_id_co>" + objEnvio.co_Origen + "</f440_id_co>" + Environment.NewLine;
            ...
            objEnvio.idDocumentoGT = 110436;
            objEnvio.strDocumentoGT = "WMS_COMPROMETER_REQUISICION_RQT";
            ...
            if (objEnvio.bitError == false)
            {
                try
                {
                    objEnvio.idDocumentoGT = 78945;
                    objEnvio.strDocumentoGT = "WMS_TRASNFERENCIA_TCO_TCA_DESDE_RQT";
