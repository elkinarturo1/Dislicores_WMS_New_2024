...
namespace Dominio
{
    public class Upload_Transferencias_Venta_TBE : IUpload
    {
        public void procesar(EstructuraUploadModel objEnvio)
        {
            ...
            if (objEnvio.bitError == false)
            {
                try
                {
                    ...
                    foreach (DataRow registro in objEnvio.dtDocumentos.Rows)
                    {
                        ...
                        strXML_GT += "<f350_notas>" + registro["f350_notas"].ToString() + "</f350_notas>";
            ...
            objEnvio.idDocumentoGT = 128212;
            objEnvio.strDocumentoGT = "WMS_TRASNFERENCIA_TBE";
