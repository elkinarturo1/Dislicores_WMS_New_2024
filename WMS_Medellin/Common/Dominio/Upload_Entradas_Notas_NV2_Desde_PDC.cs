...
namespace Dominio
{
    public class Upload_Entradas_Notas_NV2_Desde_PDC : IUpload
    {
        public void procesar(EstructuraUploadModel objEnvio)
        {
            ...
            if (objEnvio.bitError == false)
            {
                try
                {
                    ...
                    foreach (DataRow registro in objEnvio.dtCxC.Rows)
                    {
                        strXML_GT += "<CxC>";
                        strXML_GT += "<F_CIA>" + objEnvio.cia_Destino + "</F_CIA>";
                        strXML_GT += "<F350_ID_CO>" + objEnvio.co_Destino + "</F350_ID_CO>";
                        strXML_GT += "<F350_ID_TIPO_DOCTO>" + objEnvio.tipoDoc_Destino + "</F350_ID_TIPO_DOCTO>";
                        strXML_GT += "<F350_CONSEC_DOCTO>" + objEnvio.numDoc_Destino + "</F350_CONSEC_DOCTO>";
                        //strXML_GT += "<F353_VLR_CRUCE>" + registro["D_SUSR4_vlr_bruto"].ToString() + "</F353_VLR_CRUCE>";
                        strXML_GT += "<F353_FECHA_VCTO>" + registro["fechaEntrega"].ToString() + "</F353_FECHA_VCTO>";
                        strXML_GT += "<F353_FECHA_DSCTO_PP>" + registro["fechaEntrega"].ToString() + "</F353_FECHA_DSCTO_PP>";
                        strXML_GT += "</CxC>";
                    }
